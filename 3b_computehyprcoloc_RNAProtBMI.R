...
arguments <- distinct(rbind(res_pwas[,.(hgnc_symbol,study)], res_map[,.(hgnc_symbol,study)]))
...
gene_region <- dt_gene_region[hgnc == gene_to_include & study == study_copy,gene_region]
ID<- dt_gene_region[hgnc == gene_to_include & study == study_copy, id]
vcffile_exp <- paste0("/mnt/sda/gagelo01/Vcffile/Server_vcf/", ID, "/", ID, ".vcf.gz")
dat_tsmr <- gwasvcf::query_gwas(vcffile_exp, chrompos = gene_region) %>% gwasglue::gwasvcf_to_TwoSampleMR(.) %>% as.data.table(.)
...
brain_pqtl_hypercoloc <- lapply(arguments_list, function(x)
  extract_brain_pqtl(gene_to_include = x$hgnc_symbol, study = x$study, dt_gene_region = dt_gene_region )) %>%
  rbindlist(.,fill = TRUE)

#trait
gene_to_include <- arguments$hgnc_symbol %>% unique
list_dat_tsmr <- vector(mode = "list", length = length(gene_to_include))
for(i in 1:length(gene_to_include)) {
  gene_region <- dt_gene_region[hgnc == gene_to_include[i], unique(gene_region)]
  ID<-"trait-1-1"
  vcffile_exp <- paste0("/mnt/sda/gagelo01/Vcffile/Server_vcf/", ID, "/", ID, ".vcf.gz")
  dat_tsmr <- gwasvcf::query_gwas(vcffile_exp, chrompos = gene_region) %>% gwasglue::gwasvcf_to_TwoSampleMR(.) %>% as.data.table(.)
...
id_to_include_list <- purrr::cross3("Brain_Frontal_Cortex_BA9", c("rosmap", "banner", "yang"), "bmi_ukbgiant" )
...
run_hypr_and_format <- function(list_align) {
  ...
  res_hypr[, trait2 := traits %>% ifelse(grepl("rosmap", .), TRUE, .) %>% 
             ifelse(grepl("banner", .), TRUE, .) %>% ifelse(grepl("yang", .), TRUE, .)]
...
hypr_res[grepl("yang", allexposures), study := "yang"]
...
message(paste0("script finished without errors at ", Sys.time()))
